# Endpoint API Reference - Excluir Marcas pelo ID

### Descrição

Endpoint para excluir uma marca existente no sistema.
...
| `pe_brand_id`         | number |   **Sim**   | ID da marca a ser excluída     | -             |

### Exemplo de Requisição

```http
POST /brand/v2/brand-delete
...
| `recordId`   | number | ID do registro afetado (ID da marca excluída)      |
...
```json
{
    ...
    "data": [
        {
            "sp_return_id": 34,
            "sp_message": "Cadastro excluido com sucesso",
            "sp_error_id": 0
        }
    ],
    "quantity": 1,
...
Pode ocorrer se a marca não existir ou houver impedimentos para a exclusão.
