# Endpoint API Reference - Atualizar Marca (V2)

### Descrição

Endpoint para atualizar as informações de uma marca existente no sistema.
...
| `pe_brand_id`         | number |   **Sim**   | ID da marca a ser atualizada   | -             |
| `pe_brand`            | string |     Não     | Novo nome da marca             | Max 100 chars |
| `pe_slug`             | string |     Não     | Novo slug da marca             | Max 100 chars |
| `pe_image_path`       | string |     Não     | Caminho da imagem da marca     | Max 500 chars |
| `pe_notes`            | string |     Não     | Observações/Notas              | -             |
| `pe_inactive`         | number |     Não     | Status de inatividade (0 ou 1) | -             |

### Exemplo de Requisição

```http
POST /brand/v2/brand-update
...
```json
{
  ...
  "pe_person_id": 29014,
  "pe_brand_id": 28,
  "pe_brand": "Nike Edition",
  "pe_slug": "nike-edition",
  "pe_image_path": "/images/nike-v2.png",
  "pe_notes": "Atualização de linha de produtos",
  "pe_inactive": 0
}
```
...
| `recordId`   | number | ID do registro atualizado                          |
| `data`       | array  | Detalhes do retorno da procedure                   |
| `quantity`   | number | Quantidade de registros afetados                   |
...
Pode ocorrer se a marca não existir ou se houver erro de processamento no banco.

```json
{
  "statusCode": 100422,
  "message": "Error! Marca não encontrada",
  "recordId": 0,
  "data": [
    {
      "sp_return_id": 0,
      "sp_message": "Error! Marca não encontrada",
      "sp_error_id": 1
    }
  ],
  "quantity": 0,
